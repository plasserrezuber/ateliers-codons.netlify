...
1. Importer le jeu de données `gapminder.csv`  

2. Compter le nombre de pays par continent et trier les résultats par ordre décroissant

3. Effectuer les opérations suivantes à l'aide d'un pipe et les assigner dans un objet `gapminder_tmp`
...
# 2. Compter le nombre de pays par continent
gapminder %>%
  dplyr::distinct(continent, country) %>%
  dplyr::count(continent, sort = TRUE)

# 3. Nettoyer les donnnes
gapminder_tmp <- gapminder %>%
  janitor::clean_names() %>%
...
Commençons par transformer les variables qui ont un nombre de modalités limité en facteurs à l'aide de la fonction `fct()`. Par défaut, cette fonction crée des niveaux de facteur selon l'ordre d'apparition des modalités dans le tableau :  
...
La fonction `fct_infreq()` permet de trier les modalités d'un facteur selon les effectifs : 
