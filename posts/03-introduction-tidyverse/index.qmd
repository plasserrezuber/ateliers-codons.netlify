...
Le package `tidyr` proprose des fonctions permettant de transformer un tableau dans un format *tidy*.
Nos données étant déjà dans ce format, nous allons créer un sous-ensemble de données pour les transformer dans un format *non-tidy* puis les transformer à nouveau dans un format *tidy*.
