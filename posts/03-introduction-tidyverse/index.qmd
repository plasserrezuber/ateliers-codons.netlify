...
rename(pingouins, ile = Ile)
```

Nous pourrions procéder de la même façon pour plusieurs colonnes à la fois : 

```{r rename 2, echo=TRUE, eval=FALSE}
rename(pingouins, espece = Espece, ile = Ile)
```

Il existe un package, en dehors du Tidyverse, dédié au nettoyage d'un tableau (noms de colonnes, suppression des doublons, ...). Utilisons la fonction `clean_names()` de ce package :
