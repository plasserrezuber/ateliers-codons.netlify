...
Nos données contiennent 8 valeurs NA, nous en avons déjà trouvé 4 dans les premières lignes du tableau. Les 4 données manquantes restantes viennent-elles de l'absence de mesure pour un second individu ?

```{r}
pingouins_propre[is.na(pingouins_propre$longueur_bec_mm), ]
```

Nous avons ici les huit données manquantes. Nous allons supprimer ces deux individus du jeu de données : 

```{r}
pingouins_propre <- pingouins_propre[!is.na(pingouins_propre$longueur_bec_mm), ]
```

Une autre fonction permet de supprimer d'un tableau les lignes pour lesquelles il y a au moins une donnée manquante : 
