...
Les mots en **gras** sont définis dans le [glossaire](https://ateliers-codons.netlify.app/glossary.html).
...
Il dispose d'une communauté en ligne très active ainsi que d'extensions appelées [**packages**](https://ateliers-codons.netlify.app/glossary.html#package) pour presque tous les domaines et applications possibles et imaginables (cartographie, fouille de texte, modélisation, ...).

Contrairement à d'autres logiciels de statistiques comme Minitab ou SPSS, R peut être exécuté entièrement en ligne de [commande]((https://ateliers-codons.netlify.app/glossary.html#commande)) et ne nécessite pas d'interface graphique, ce qui lui confère une grande flexibilité.
...
-   C'est un langage de programmation plutôt qu'une interface graphique, ce qui permet à l'utilisateur de sauvegarder facilement des [**scripts**](https://ateliers-codons.netlify.app/glossary.html#script) pour les ré-utiliser ou les partager
...
Quand vous démarrez une nouvelle session dans RStudio, la [console](https://ateliers-codons.netlify.app/glossary.html#console) affiche également des informations comme la version de R présente sur votre ordinateur ainsi que des indications sur la manière d'obtenir de l'aide.
...
Vous y trouverez votre [**environnement**](https://ateliers-codons.netlify.app/glossary.html#environnement) (données importées, [objets](https://ateliers-codons.netlify.app/glossary.html#fonction), [fonctions](https://ateliers-codons.netlify.app/glossary.html#fonction), ...), ainsi que d'autres onglets comme l'historique des commandes exécutées ou l'accès à des tutoriels (pour les versions récentes de RStudio).
...
- *trier les données* : nous aimerions que les barres soient triées par ordre décroissant. 
La fonction `sort()` permet de trier des données, avec l'argument  `decreasing = TRUE` pour modifier l'ordre (croissant par défaut)
