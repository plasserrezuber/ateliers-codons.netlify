...
Vous pouvez diviser votre script en différentes parties pour simplifier la lecture.
Pour cela, ajoutez des titres sous forme de commentaires et accolez des tirets (au moins quatre) pour signaler à R qu'il s'agit de divisions dans votre script.
...
```{r eval=FALSE}
pingouins_propre[, c(3, 6:9)]
```

-   supprimer les colonnes inutiles

```{r eval=FALSE}
pingouins_propre[, -c(1:2, 4:5)]
```
...
```{r eval=FALSE}
pingouins_propre[, c("espece", "longueur_bec_mm", "hauteur_bec_mm",
...
```{r}
pingouins_propre <- pingouins_propre[, c("espece", "longueur_bec_mm",
...
# Individus dont la masse est d'au moins 6 kgs et le bec mesure moins de 50 mm de long
pingouins_propre[pingouins_propre$masse_corporelle_g >= 6000 & pingouins_propre$longueur_bec_mm >= 50, ]
```
...
La fonction `agregate()` permet de calculer des statistiques descriptives (par exemple la moyenne, la médiane, ...) d'une variable en regroupant les données.

Cette fonction s'utilise de la manière suivante : 

`agregate(x ~ y, data, FUN)` : 
