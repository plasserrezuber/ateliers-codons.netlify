...
Git a ajouté une branche au niveau du précédent commit, et nous nous trouvons à présent sur cette branche. Pour le vérifier, nous pouvons taper la commande `git branch` dans le terminal : 

![](img/git_cmd_08.png)

Nous voyons qu'il existe maintenant deux branches : 
- *(HEAD detached at #####)* : nouvelle branche au niveau du commit précédent  
- *master* : la branche principale

Si nous tapons la commande `git log`, nous voyons qu'il n'y a que deux commits dans l'historique (nous sommes revenus un commit en arrière).

Ajoutons une ligne de texte dans le fichier `README.md` sur cette nouvelle branche : 

![](img/git_cmd_09.png)

Sauvegardons le fichier et effectuons un `add` puis un `commit`.
